...
# at each increment update list of coordinates and height, check if latter & value of former match, find
## Part 1: unique positions of 9
## Part 2: unique paths that reach 9
for start_row, start_col in trailheads:

    """ Part 1:
    queue =  deque([(start_row, start_col, 0)])  #height initialised at 0 at trailhead
    visited = set()
    nines_reached = set() """
    ...
    while queue:
        #row, col, height = queue.popleft() #Part 1
        row, col, height, path = queue.popleft()
        ...
        # iterate through 4 directions, if within grid & single increment repeat iteration, update list
        new_height = height + 1
        for r, c in directions:
            new_row, new_col = row + r, col + c
            
            if 0 <= new_row < len(lines) and 0 <= new_col < len(lines[0]):
                if lines[new_row][new_col] == str(new_height):
                    #queue.append((new_row, new_col, new_height)) #Part 1
                    new_path = path + [(new_row, new_col)]
                    queue.append((new_row, new_col, new_height, new_path))
